function Gyyy_2d = Gyyy_2d_op(sigma, dx, dy)
%function Gyyy_2d = Gyyy_2d_op(sigma, dx, dy)
%
% d^3G/dy^3 operator with operator shift
%
...
d3G_y = (y-dy).*(3*sigma^2-(y-dy).^2).*exp(-(y-dy).^2/(2*sigma^2))/sqrt(2*pi)/sigma^7;
Gyyy_2d = G_x.*d3G_y;

% % for debug only
% figure;
% axis([-w w -w w]);
% imagesc([-w:w],[-w:w], Gyyy_2d); colormap gray;
% hold on;
