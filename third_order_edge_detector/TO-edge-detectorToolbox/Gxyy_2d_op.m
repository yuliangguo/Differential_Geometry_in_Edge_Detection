function Gxyy_2d = Gxyy_2d_op(sigma, dx, dy)
%function Gxyy_2d = Gxyy_2d_op(sigma, dx, dy)
%
% d^3G/dxd^2y operator with operator shift
%
...
dG_x = -(x-dx).*exp(-(x-dx).^2/(2*sigma^2))/sqrt(2*pi)/sigma^3;
d2G_y = ((y-dy).^2-sigma^2).*exp(-(y-dy).^2/(2*sigma^2))/sqrt(2*pi)/sigma^5;
Gxyy_2d = dG_x.*d2G_y;

% % for debug only
% figure;
% axis([-w w -w w]);
% imagesc([-w:w],[-w:w], Gxyy_2d); colormap gray;
% hold on;
